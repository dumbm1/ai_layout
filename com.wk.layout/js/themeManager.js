...
var themeManager = (function () {
  ...
  function updateThemeWithAppSkinInfo (appSkinInfo) {
    ...
    var btnsMarg = document.getElementsByClassName ('label-marg');
    ...
    if (panelBgColor.red > 122) {
      fontColor = "000000";
      //@MaratShagiev(c)  sprite buttons
      for (var i = 0; i < mainBtns.length; i++) {
        var btn       = mainBtns[i];
        btn.className = 'main-btn main-btn-dark';
      }

      $ ('.btn-link').removeClass ('btn-link-light').addClass ('btn-link-dark');

      for (i = 0; i < btnsMarg.length; i++) {
        var obj1       = btnsMarg[i];
        obj1.className = 'left-label label-marg label-marg-dark';
      }
    } else { //@MaratShagiev(c) sprite buttons
      for (var i = 0; i < mainBtns.length; i++) {
        var btn       = mainBtns[i];
        btn.className = 'main-btn main-btn-light';
      }

      $ ('.btn-link').removeClass ('btn-link-dark').addClass ('btn-link-light');

      for (i = 0; i < btnsMarg.length; i++) {
        var obj1       = btnsMarg[i];
        obj1.className = 'left-label label-marg label-marg-light';
      }
    }
    ...
    addRule (styleId, ".hostLabel", "background-color:" + "#" + bgdColor);
    addRule (styleId, ".hostLabel:hover", "background-color:" + "#" + bgdColor);
    addRule (styleId, ".hostLabel:active", "background-color:" + "#" + darkBgdColor);
    addRule (styleId, ".hostLabel", "border-color: " + "#" + lightBgdColor);

    //@MaratShagiev(c) the main buttons style
    addRule (styleId, ".main-btn", "background-color:" + "#" + bgdColor);
    addRule (styleId, ".main-btn:hover", "background-color:" + "#" + darkBgdColor);
    /* addRule ( styleId, ".main-btn:hover", "border-color:" + "#" + darkBgdColor );
     addRule ( styleId, ".main-btn:hover", "border-width:" + "1px" );
     addRule ( styleId, ".main-btn:hover", "border-style: solid" );*/
    addRule (styleId, ".main-btn:active", "background-color:" + "#" + bgdColor);
    /*  addRule ( styleId, ".main-btn:active", "border-color:" + "#" + bgdColor );
     addRule ( styleId, ".main-btn:active", "border-width:" + "1px" );
     addRule ( styleId, ".main-btn:active", "border-style: solid" );*/
